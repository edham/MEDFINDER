...
public class clsSesionVigilanciaDAO {
    ...
    public static int Agregar(Context context,clsSesionVigilancia entidad,int principal)
    {
        int id = 0;
        ...
        if(bd!=null)
        {
            ...
            registro.put("int_principal",principal);
    ...
    public static clsSesionVigilancia BuscarPrincipal(Context context)
    {
        ...
        if(bd!=null)
        {
            String query="select sv.int_id,sv.dat_fec_ini,sv.int_id_cuadrante,cu.str_nombre,cu.str_descripcion,sv.dat_fec_vigilancia from "
                    +NOMBRE_TABLA+" as sv inner join CUADRANTE as cu on cu.int_id=sv.int_id_cuadrante where int_principal=1";
    ...
    public static clsSesionVigilancia BuscarId(Context context,int id)
    {
        clsSesionVigilancia entidad=null;
        bdSQLite admin=new bdSQLite(context,null);
        SQLiteDatabase bd=admin.getWritableDatabase();
        ...
        {
            String query="select sv.int_id,sv.dat_fec_ini,sv.int_id_cuadrante,cu.str_nombre,cu.str_descripcion,sv.dat_fec_vigilancia from "
                    +NOMBRE_TABLA+" as sv inner join CUADRANTE as cu on cu.int_id=sv.int_id_cuadrante where sv.int_id="+id;
